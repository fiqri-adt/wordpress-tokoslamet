...
class Woocommerce_Confirm_Payment_Public {
  ...
  public function confirm_payment() {
    ...
    if ( $this->options['transfer_date_disabled'] ) {
  ...
  public function check_order( $order_id ){
    ...
    if ( $orders ) {
      ...
      $allowed_payment_method = apply_filters( 'wcp_allowed_payment_method', array('bacs') );

      if ( in_array($orders->get_payment_method(), $allowed_payment_method) ) {
  ...
  public function payment_complete( $payment_id ){
    ...
    if ( $this->options['line_notification_enabled'] && $line_notify_token = $this->options['line_notify_token'] ) {
      ...
      $fields        = apply_filters( 'wcp_line_notify_message_fields', $this->line_notify_message( $payment_id ), $orders, $payment_id );
      
      foreach ( $fields as $field ) {
        $message .= $field['label'] . ' : ' . $field['value'] . PHP_EOL;
      }

      $this->line_notify( PHP_EOL . $message, $slips_thumb, $slips, $line_notify_token );
  ...
  public function line_notify_message( $payment_id ){
    ...
    $fields = array(
      'name' => array(
        'label' => __( 'Name', 'woocommerce-confirm-payment' ),
        'value' => $name
      ),
      'phone' => array(
        'label' => __( 'Phone', 'woocommerce-confirm-payment' ),
        'value' => $phone
      ),      
      'date' => array(
        'label' => __( 'Date', 'woocommerce-confirm-payment' ),
        'value' => date_i18n( get_option('date_format') . ' ' . get_option('time_format'), strtotime( $date ) )
      ),
      'amount' => array(
        'label' => __( 'Amount', 'woocommerce-confirm-payment' ),
        'value' => number_format($amount)
      ),
      'bank' => array(
        'label' => __( 'Bank', 'woocommerce-confirm-payment' ),
        'value' => $bank
      ),
      'order_id' => array(
        'label' => __( 'Order', 'woocommerce-confirm-payment' ),
        'value' => $order_id
      ),
    );
  ...
  public function line_notify( $message, $imageThumbnail, $imageFullsize, $token ){
    ...
    $response = wp_remote_post( $this->line_api, array(
      'method' => 'POST',
      'headers' => array(
  ...
  public function add_payment_form_thankyou( $order_id ) {
    if( !empty( $this->options['show_form_on_thankyou_page'] ) ) {
      echo '<h2 class="wc-bacs-bank-details-heading">'. esc_html__( 'Confirm payment', 'woocommerce-confirm-payment' ) .'</h2>';
      echo do_shortcode( '[wcp_confirm_payment_form order_id="'. $order_id .'"]' );
    }
  }
